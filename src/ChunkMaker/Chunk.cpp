...
/*   Updated: 2025/08/16 13:04:41 by mbirou           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Chunk::Chunk(const glm::vec3 &nPos) : rendered(false), _used(false), _created(false), _needRemesh(false), _edited(false), _generated(false), _generating(false), _uploaded(false)
{
	_minHeight = 255;
	_maxHeight = 0;
	_indicesSize = 0;
...
void	Chunk::genMesh()
{
	...
	sideChunks[0] = (sideChunks[0] && sideChunks[0]->_created && sideChunks[0]->setUsed() ? sideChunks[0] : NULL);
	sideChunks[1] = (sideChunks[1] && sideChunks[1]->_created && sideChunks[1]->setUsed() ? sideChunks[1] : NULL);
	sideChunks[2] = (sideChunks[2] && sideChunks[2]->_created && sideChunks[2]->setUsed() ? sideChunks[2] : NULL);
	sideChunks[3] = (sideChunks[3] && sideChunks[3]->_created && sideChunks[3]->setUsed() ? sideChunks[3] : NULL);
	...
	if (sideChunks[0] && sideChunks[1] && sideChunks[2] && sideChunks[3])
		_needRemesh.store(false);
	else
		_needRemesh.store(true);
...
(pos.x + (31 - z), pos.z + 32)))
(pos.x + (31 - z), pos.z - 1)));*/
// preset of the positions for the edgeMask;
glm::ivec3	offsets[4] = {
	glm::ivec3(-1, 0, 0),
	glm::ivec3(32, 0, 0),
	glm::ivec3(0, 0, 32),
