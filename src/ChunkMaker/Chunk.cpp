...
*/
void	Chunk::placeBlock(glm::ivec3 &chunkPos, const std::vector<uint64_t> &usedData, uint64_t &slice, uint64_t &westFaces, uint64_t &eastFaces, uint64_t &northFaces, uint64_t &southFaces)
{
	...
	if (northFaces & 3 || southFaces & 3) // skipping full air
	{
	...
	if (westFaces & 3 || eastFaces & 3 || slice & 3) // same as the other one
	{
		...
		if (slice & 3 // top face
			&& ((chunkPos.y + 1) * 32 + chunkPos.z >= (int)usedData.size()
				|| ((usedData[(chunkPos.y + 1) * 32 + chunkPos.z] >> (chunkPos.x * 2)) & 3) != (slice & 3)))
			addVertices(block, V1 + chunkPos, V2 + chunkPos, V4 + chunkPos, V3 + chunkPos, 2);

		if (chunkPos.y != 0 && slice & 3
			&& (chunkPos.y - 1) * 32 + chunkPos.z < (int)usedData.size()
...
void	Chunk::genMesh()
{
	...
	uint64_t	edges[8]; // these are the slices of adjacent chunks (4-7 is for water only) 0 = west, 1 = east, 2 = north, 3 = south
	...
	for (chunkPos.y = 0; chunkPos.y <= std::max(_maxHeight, (uint8_t)WATERLINE); ++chunkPos.y)
	{
		...
		for (chunkPos.z = 0; chunkPos.z < 32; ++chunkPos.z)
		{
			...
			ground.northSlices = fatCulling(ground.rotSlice, true, ((edges[2] >> ((31 - chunkPos.z) * 2)) & 3));
			ground.southSlices = fatCulling(ground.rotSlice, false, ((edges[3] >> ((31 - chunkPos.z) * 2)) & 3));

			if (pos == glm::vec3(1824, 0, 576))
			{
				std::bitset<64> sl(ground.slice);
				std::bitset<2> ed(((edges[0] >> ((31 - chunkPos.z) * 2)) & 3));
				std::bitset<64> cl1(ground.slice << 2);
				std::bitset<64> cl2((ground.slice << 2) | ((edges[0] >> ((31 - chunkPos.z) * 2)) & 3));
				std::bitset<64> cl3 (~((ground.slice << 2) | ((edges[0] >> ((31 - chunkPos.z) * 2)) & 3)));
				std::bitset<64> cl(ground.westFaces);
				std::cout << sl << ":" << ed << std::endl << cl1 << std::endl << cl2 << std::endl << cl3 << std::endl << cl << std::endl << std::endl;
			}
...
void	Chunk::genChunk()
{
	...
	for (int z = 0; z < 32; ++z)
	{
		...
		// {
		// 	std::bitset<64> a(ChunkMask[10 * 32 + z]);
		// 	std::cout << a << std::endl;
		// }
	}
